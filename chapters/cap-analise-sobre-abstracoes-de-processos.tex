\chapter{Análise Sobre Abstrações de Processos}
...
Este trabalho se diferencia dos \hltodo[demais]{Não entendi, quais? Os que você menciona no cap 3?} por considerar o uso de novas extensões
...
\todo[inline]{Achei um pouco longo e repetitivo, acho que dá para cortar um pouco}

Quando a academia faz uma nova proposta para aplicações que estão no \hltodo[estado da
prática]{Como a gente viu, essa expressão é bem informal e ``marketeira''; eu colocaria entre aspas ou trocaria por outra coisa}, ela deve ser genérica o bastante para ser utilizada por
...
tornam a ampla adoção de uma determinada melhoria \hltodo[impraticável.]{Melhor ou dizer ``difícil'' ou mudar para ``às vezes impraticável''}
...
alterações na semântica das aplicações em espaço de usuário para que elas
possam tirar proveito de alguma melhoria. Para esses casos, \hltodo[desconsideramos]{O que significa ``desconsideramos''? Você coloca a proposta que tem isso como inviável? Você considera que se é só uma função não é um problema significativo em termos de compatibilidade? Não entendi}
...
controle fino do gerenciamento de recursos e consequentemente o \hltodo[desacoplamento]{Acho que tem que seguir o modelo dos demais: escrever ``desacoplamento do gerenciamento de recursos'' e colocar em negrito}
...
das ideias apresentadas por esse trabalho estejam presentes em alguns SOs, as
...
sugerem \textbf{desacoplar o VAS dos processos}\todo{seria bom ser ``desacoplamento'' ao invés de ``desacoplar''} e permitir que eles tenham
...
Light-weight Context (lwC) indiretamente propõe o total
...
como criar \emph{snapshots} do estado atual do processo e depois reverter o
...
contexto do processo (o que é razoavelmente caro), a própria aplicação
pode escolher quando e qual contexto manipular e fornecer, assim, um
controle fino para os programadores (o que pode melhorar o desempenho e a
segurança).
...
\todo[inline]{Acho que vale mudar um pouco esse texto e dizer ``neste trabalho falamos dessa porra toda e percebemos esse lance de desacoplar para todo lado, então resolvemos organizar a bagaça''. Acho que você quis dizer exatamente isso neste parágrafo, mas você já começou dizendo ``este trabalho consiste em analisar os impactos do desacoplamento'', quando na verdade esse é o lugar ao qual você chegou no fim do caminho.}
...
context. Nas camadas mais externas temos o VAS, que é um elemento importante
para que os processos em SOs de propósito geral funcionem de forma apropriada
(dada sua importância, ele ocupa boa parte da representação da abstração).
...
correto funcionamento da abstração, mas são fundamentais para diversas
aplicações modernas.

\todo[inline]{Nesta figura tem um ``??''}
...
possibilidade de restringir qual operação de virtualização deseja-se utilizar.
  ...
  \begin{itemize}
    ...
    \begin{itemize}
    \item \texttt{Options}: Recebe opções que \hltodo[restringem]{Restringe ou determina?} o modo de virtualização.
    \end{itemize}
...
técnica, torna-se necessária uma função especial para realizar a \hltodo[alocação]{Alocação de memória?}
...
operação solicitada pelo usuário.\todo{Entendi, mas a explicação está feinha. É só reescrever um pouco mais claro} Por sua vez, tal função seletivamente realiza
...
essa função como padrão. Note que ela se comporta como um adaptador
...
operação de fotografar o estado para que ela seja \hltodo[recursiva]{?! Recursão em quê?} ou não\footnote{O
...
\texttt{if} falhar, então a troca de contexto encerra devolvendo
...
metadados sobre os \hltodo[processos a fim de garantir a consistência entre os processos.]{Além da repetição da palavra processos, não entendi por que precisa disso (ou melhor, não entendi o que significa ``consistência entre os processos''; quais processos? Os novos? Os novos e o velho? Os novos se houver mais de um?}
Na Linha~\ref{line:persistenciaCxtInstance}, notamos que foi passada uma
...
\todo[inline]{Você marcou um TODO aqui e, de fato, ou você explica como nos demais ou muda um pouco o texto para só fazer um comentário do tipo ``essa mesma bagaça pode ser utilizada, com modificações, para implementar live patching''}
...
desacoplamento do VAS. A ideia do \emph{live patching} já existe e consiste em
...
dos processos oferece diversos recursos e novos modelos de programação,
...
apresentadas nesta subseção, em conjunto com o conceito de desacoplamento do
VAS apresentado pelo SpaceJMP. Entretanto, o \emph{bead} leva a um nível mais avançado o controle
do desacoplamento do VAS, pois permite o controle fino sobre cada um dos seus elementos.
...
operação de troca). Dentro do compartimento, não é possível utilizar as operações de \hltodo[alocação]{Alocação de memória?}
