\imgchapter[right,width=8cm,bleed=19pt,shift=37pt]{chapter_6_title}{Análise Sobre Abstrações de Processos}
...
\input{algorithm/lib_bead_snapshot.tex}
...
\input{algorithm/lib_bead_virt_mode.tex}
...
atualização em tempo de execução. Na seção~\ref{sec:ate} foi descrito em
detalhes o comportamento do padrão ATE, lembre-se que essa técnica precisa
tratar o recebimento do pedido de atualização feito no espaço de usuário; note
que mesmo utilizando a \emph{libbead}, ainda é preciso realizar o pedido de
atualização.

\input{algorithm/lib_bead_live_patch.tex}
