...
complexos em que cada uma das suas partes é \hltodo[perfeitamente]{prefiro ``estritamente''; nada é perfeito} definida baseada nas
...
arquivo, \hltodo[oferecem mecanismos para que as aplicações de
usuários utilizem os recursos de hardware]{meio repetido com o que veio antes}~\citep{tanenbaum}.

\todo[inline]{Aqui acho que é importante frisar \emph{por que} vale a pena melhorar a abstração de processos; não precisa ser algo longo, mas o próximo parágrafo está resumido demais}
...
para atender às demandas de \hltodo[hardware]{``novos'' hardwares, precisa explicar melhor; esta frase inteira, na verdade, podia aumentar um pouco}, segurança, escalabilidade, modelos de
...
\todo[inline]{Até aqui foi um assunto (``sistemas operacionais são importantes, processos são uma abstração importante então vamos investigar as novidades nessa área''). Agora vem outra coisa: ``há três tipos de trabalhos por aí que normalmente se auto-avaliam em duas fases: (1) com microbenchmarks e (2) com uma aplicação de exemplo''. Acho que cai bem criar uma subseção aqui explicando essa ideia}
...
consiste na criação de um SO totalmente novo \hltodo[e sem qualquer dependência]{Não entendi}.
...
pequenos pedaços da aplicação} \todo{Acho que não concordo com essa definição de microbenchmark!} para avaliar o desempenho das suas propostas. A
...
\todo[inline]{Aqui temos uma nova subseção, que é ``objetivos'' ou algo assim: ``dado que processos são legais e que temos um entendimento de como as pesquisas na área são feitas, perguntamos e respondemos isto aqui''.}
...
Kernel Linux. Por isso, no Capítulo~\ref{cap:fundamentacao} \hltodo[descreveremos
conceitos mais aplicáveis ao Linux que aos outros SOs disponíveis]{Não entendi e não sei se parece boa ideia}. Para
...
investigamos \hltodo[diferentes técnicas de implementação]{acho que aqui poderia citar novamente o lance da divisão em três categorias}, descritas no Capítulo
...
das quatro perguntas de pesquisa, \hltodo[derivamos um conjunto de propriedades
desejáveis para a próxima geração de abstrações de processos]{Mas isso é exatamente a RQ1!}. Por fim, no
