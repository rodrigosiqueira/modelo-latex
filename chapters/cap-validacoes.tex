...
\todo[inline]{Olhando por alto, acho que a organização está ok, mas se quiser mudar,
eu mexeria na introdução e faria uma argumentação nesta linha:

O SO dá a infra para rodar os processos. Ele precisa garantir (1) acesso aos recursos,
(2) desempenho, (3) segurança/isolamento e (4) funcionalidades adicionais (p.ex.,
mutexes). Melhorias no SO devem trazer benefício em alguma dessas frentes sem
trazer muito overhead.

Para testar essa bagaça, microbenchmarks e aplicações ``reais''. Aplicações reais
podem consumir CPU, podem ler muito do disco, podem escrever muito no disco, podem
ler muito da memória, podem escrever muito na memória, podem gerar muito tráfego
de rede, podem gerar muitas trocas de contexto (pela paralelização), podem depender
de sincronização e podem ferrar com os caches de memória e de disco. Além disso,
algumas aplicações podem ter necessidades específicas de segurança. Idealmente,
seria bom testar com aplicações que exercitam todas essas coisas.

Na prática, queremos algo que gere uma carga razoável de CPU, disco, rede e
paralelismo. Não faz muita diferença para avaliar a qualidade do SO se as
operações de I/O são de escrita ou leitura, então qualquer caso serve. Se o que
está sendo testado não interfere com as primitivas de sincronização, também dá
para deixar de lado esse aspecto. O mesmo vale para caches, e ainda por cima
eles são muito imprevisíveis então dá para deixar de lado também. Finalmente,
seria bom algo que é relativamente fácil de gerar carga e medir o desempenho.

O que atende bem esses requisitos são servidores web: são altamente paralelos,
geram bastante I/O e tráfego de rede, dependem de algum processamento na CPU
e há ferramentas para gerar carga e medir o desempenho. No caso da segurança,
faz sentido usar uma aplicação dedicada, como gpg ou openssh, e/ou uma biblioteca,
como openssl. Não por acaso, quase todos os trabalhos que avaliamos usam alguma
dessas ferramentas para fazer suas validações.

Não vi a parte de microbenchmarks, mas talvez você também possa argumentar
algo como ``se é para validar com microbenchmarks, precisamos ter um conjunto
mínimo desses microbenchmarks que todo mundo usa para minimizar as chances de
...
\todo[inline]{Tem outra pergunta que precisa ser respondida antes: ``Como faz para avaliar de maneira mais completa uma nova abstração?''. É muito fácil acrescentar uma frase já respondendo que o melhor é usar aplicações reais, aí você pode levantar a pergunta ``quais aplicações''. Você fez ao contrário: pergunta ``quais aplicações podemos usar'' antes de dizer ``o negócio é usar aplicações existentes''.}
\begin{quote}
...
\hltodo[buscamos por programas]{Isso não é muito verdade, né? Você está colocando aqui os programas que os autores dos papers usaram. Prefiro deixar claro que os caras é que escolheram e você está explicando o porquê.} que auxiliam a demonstrar que uma nova proposta de
abstração de processos é valida ou não. Buscamos três características: estresse
que a aplicação pode gerar, possíveis falhas de segurança e \hltodo[mecanismos
periféricos]{Não captei wtf}.  Aplicações que consomem muitos recursos de hardware são ideais
...
por exemplo, novas formas de compartilhamento de memória e otimizações.\todo{``Nesses casos, é interessante utilizar alguma aplicação do tipo adequado para a validação''}
...
vez que são rápidos e \hltodo[ajudam durante o desenvolvimento]{Está meio jogado isso}. Um conjunto de
...
consegue revelar \hltodo[a real natureza]{Meio esquisito, prefiro algo como ``não consegue revelar possíveis efeitos colaterais ou interferências com outras funcionalidades''} de uma alteração.
...
\hltodo[software]{Microbenchmarks também é software; prefiro ``programas'', ``aplicações'' ou ``programas representativos''etc.} e \textit{microbenchmarks}. Primeiramente apresentamos programas que
são alvos \hltodo[ideais]{Que tal ``favoráveis''?} para serem adaptados de forma a utilizar as novas abstrações
...
ser utilizados para validar novas extensões nos processos. \hltodo[Em resumo, esse
...
qual \textit{Event} opera é similar a \textit{Worker}, \hltodo[com a diferença de
...
\begin{itemize}
  ...
  \item Processos \textit{Worker}, que fazem o trabalho de manipular conexões, ler e escrever em disco e comunicar-se com os \hltodo[servidores.]{Clientes, não?}
\end{itemize}
...
\textit{dump} dos dados no momento em que foi solicitado.\todo{Parece igual ao RDB mas só quando ``alguém'' pede, é isso mesmo?}
...
Apache trabalha com diversos \textit{plugins} que são acoplados ao código
...
Por fim, é importante ressaltar que vários dos \textit{microbenchmarks} devem
