...
processos e os vários temas que órbitam tal conceito (threads, \emph{fork},
...
Making memory available and usable for user applications represents one of the
core OS duties, and most OSes offer the illusion of full memory availability by
decoupling the physical memory from how processes see it. Processes only see a
Virtual Address Space (VAS), which is mapped by the OS to the physical memory,
guaranteeing good isolation among processes. For handling VASes and offering a
useful feature for user applications, OSes have to adopt a specific memory
model;  currently, most of the production-oriented OSes and hardware broadly
support the page-base memory management model. This model divides the VAS and
the physical address space of each process into a set of pages, which is a
small range of contiguous addresses with a fixed size, start address, and
permissions. The page memory model has some attractive advantages: permission
control at the page size level, mechanisms for data sharing, fast protection
checking, accurate notifications about protection violations, and the
possibility of mapping memory to disk.

The approach of using a single address space per process has proven to be
efficient over the years but, despite its success, it is not flawless and is
still open for improvements. First, the single linear address space approach
isolates each process in the memory, which enhances system reliability and
security. Nevertheless, the process has virtually no way to restrict its own
access to some of its memory segments, which might be useful to reduce the
security risk of using third-party binary code. Second, control of memory
sharing is limited to the page size; this makes data sharing less efficient and
reduces the programming possibilities in user space. Finally, the coarseness of
page-level protection creates opportunities for malicious exploits, such as
buffer and stack overflows or code execution in shared libraries.
